...
// ValidationService handles CUE validation for the review workflow
type ValidationService struct {
	cascadeValidator *validator.CascadeValidator
	schemaPath       string
}
...
func NewValidationService(schemaPath string) (*ValidationService, error) {
	...
	cascadeValidator, err := validator.NewCascadeValidator(schemaPath)
	if err != nil {
...
func (vs *ValidationService) ValidateDataAgainstSchema(data interface{}, schemaName string) *ValidationResult {
	...
	// Convert cascade validation result to review validation result
	return vs.convertCascadeResult(result, schemaName)
}

// convertCascadeResult converts a cascade validation result to a review validation result
func (vs *ValidationService) convertCascadeResult(cascadeResult *validator.ValidationResult, intendedSchema string) *ValidationResult {
...
func (vs *ValidationService) buildFallbackMessage(cascadeResult *validator.ValidationResult) string {
...
// convertValidationErrors converts cascade validation errors to review errors
func (vs *ValidationService) convertValidationErrors(cascadeErrors []validator.ValidationError) []string {
	var errors []string
...
// convertCascadeAttempts converts cascade attempts to review format
func (vs *ValidationService) convertCascadeAttempts(cascadeAttempts []validator.CascadeAttempt) []CascadeAttempt {
	var attempts []CascadeAttempt
	for _, attempt := range cascadeAttempts {
		reviewAttempt := CascadeAttempt{
...
func (vs *ValidationService) GetValidationSummary(result *ValidationResult) string {
	if result.Valid {
...
func (vs *ValidationService) GetDetailedValidationReport(result *ValidationResult) string {
...
// ValidationResult represents the result of validating data against a schema
type ValidationResult struct {
	...
	CascadeAttempts  []CascadeAttempt  `json:"cascade_attempts"`
}
