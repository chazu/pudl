...
func (e *Executor) Run(ctx context.Context, opts RunOptions) (*RunResult, error) {
	...
	// Look up model
	modelInfo, err := e.modelDisc.GetModel(def.ModelRef)
	if err != nil {
		return nil, fmt.Errorf("model %q for definition %q: %w", def.ModelRef, opts.DefinitionName, err)
	}
...
func (e *Executor) ListMethods(defName string) ([]MethodStatus, error) {
	...
	modelInfo, err := e.modelDisc.GetModel(def.ModelRef)
	if err != nil {
		return nil, fmt.Errorf("model %q: %w", def.ModelRef, err)
	}
